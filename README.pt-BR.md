# MermaidFlow Animator — Plugin Obsidian
...
| `controls` | `true` &#124; `false` &#124; `show` &#124; `hide` | `true` | Mostra/esconde o botão de play/pause no canto superior esquerdo. Útil para gravar GIFs limpos. |
...
No **canto superior esquerdo** de cada diagrama aparece um botão `⏸` (pause) / `▸` (play). Clicar nele pausa **toda** a animação — partículas congelam onde estão, novos spawns ficam suspensos. Clicar de novo retoma sem saltos (o `dt` é resetado pra evitar avanços bruscos).
...
**Não suportado** (ainda): subgraphs, classes/styles, click handlers, gráficos não-flowchart (sequence, gantt, pie, etc.).
...
Alternativa: use o [BRAT plugin](https://github.com/TfTHacker/obsidian42-brat) e adicione este repositório como beta — o BRAT mantém atualizado automaticamente.
...
├── LICENSE                ← MIT
└── README.md              ← (en) este arquivo é a versão pt-BR
```
...
Sem React, sem virtual DOM. ~105 KB minificado (dagre é ~80 KB).
...
2. **Extraia** a pasta `plugin/` do monorepo para a raiz desse repo via `git subtree`:
   ```bash
   ...
   ```
3. **Faça push da primeira tag** — a GitHub Action builda e cria a release com os assets:
   ```bash
   git tag 0.1.0
   git push --tags
   ```
...
7. **Após o merge**, o plugin aparece automaticamente na aba Community plugins do Obsidian de qualquer usuário.
...
Os arquivos em `shared/` são **cópias** dos correspondentes em `../src/` da web app. Se você atualizar o parser/layout/cores na web app, copie para cá:
